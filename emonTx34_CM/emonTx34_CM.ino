/*BB July 4, 2023
 * For version 0-2-K of emonPlug
 * Changes needed for emonPlug should be documented here.
 * 
 * Changed default node from 15 (used in my setup already) to 28.
 * Changed network group to 100:   byte networkGroup = 100;  
 * 
 * 
 * sendWithRetry was not working for me, so changed the following:
 ...
 *  float i1Lead = 7.6;                                      // 4.2° phase 
 *  
 * Added this to list_calibration in _config: if (USA) Serial.print(F("USA mode enabled. vCal_USA = ")); Serial.println(EEProm.vCal_USA); 
*/
...
struct {
  ...
  byte  nodeID = 28;                                       // node ID for this emonTx.
  ...
  float vCal_USA = 113.88;                                  // (120V × 13) / 12.0V = 130.0  Calibration for US AC-AC adapter 77DA-10-09 
